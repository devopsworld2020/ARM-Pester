...
Describe "Virtual Machine Tests" -tag "AzureInfrastructure" {
    context "VM Tests" {
        ...
        it "Virtual Machine $vmName Should Be Size Standard_B2s" {
            $vm.HardwareProfile.VmSize | should be "Standard_B2s"
        }

        it "Virtual Machine $vmName Should Be Located in East US" {
            $vm.Location | should be "eastus"
        }
